...
- **504-instrument universe** — stocks, sector/factor/bond/commodity ETFs, and crypto (BTC/ETH ETFs).
- **Scenario compare** — every run is snapshotted; flip between them or view a side-by-side delta table.
- **Ticker autocomplete** — search 504 instruments by symbol *or* company name.
...
```
        ...
        └──────────────┬───────────────┘
                       ▼
        ┌──────────────────────────────┐
        │  api/prices.csv (504 tickers)│  ← bundled adjusted-close history
...
│   ├── prices.csv              ← bundled adjusted-close history (504 tickers)
...
| `GET`  | `/tickers`   | The 504-instrument universe (symbol + company name) for the picker |
...
| `GET`  | `/snaptrade/positions` | Positions reconciled against the 504-symbol universe |
...
Positions are reconciled server-side against the 504-symbol universe and the
...
**Universe:** 504 instruments (stocks · sector/factor/bond/commodity ETFs · crypto ETFs)
